...
(def function compute-cgi-environment (start-environment script-path &key extra-path www-root)
  (bind ((request-uri (uri-of *request*))
         ;;(request-uri-path (hu.dwim.uri:path-of request-uri))
    ...
    (labels ((set (name value)
      ...
      (set "QUERY_STRING"    (hu.dwim.uri:query-of request-uri))
      ...
      (set "SERVER_NAME"     (hu.dwim.uri:host-of request-uri))
      (set "SERVER_PORT"     (aif (hu.dwim.uri:port-of request-uri)
