...
(def special-variable *session-application* (make-instance 'standard-application :path "session"))
          ...
          (make-raw-functional-response ()
            (emit-http-response/simple-html-document ()
              <p "We have a session now... "
                 ...
                 <a (:href ,(string+ (join-strings (path-of *application*) #\/) "delete/"))
                    "delete session">>)))
        (bind ((application *application*)) ; need to capture it in the closure
          (make-raw-functional-response ()
            (emit-http-response/simple-html-document ()
              <p "There's no session... "
                 <a (:href ,(string+ (join-strings (path-of application) #\/) "new/"))
                    "create new session">>))))))
...
(def entry-point (*session-application* :path "delete/")
  (bind ((old-session nil))
    ...
    (make-redirect-response (join-strings (path-of *application*) #\/))))
